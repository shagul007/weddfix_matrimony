  ...
  <div class="container">
   <div class="breadcrumb1">
     <ul>
        ...
        <li class="current-page">Viewed &amp; Not Contacted Profiles</li>
     </ul>
   </div>
   <div class="col-md-9 profile_left2">
     <form>
	  <div class="form_but2">
		<p class="control-lable1" for="sex">These are profiles viewed and not contacted by you. These profiles exactly match your <a href="update_partner_preference">Partner Preference</a> </p>
		<div class="clearfix"> </div>
	  </div>
	 </form>
	 <table id="pagination">
	 <c:if test="${!viewedAndNotContactedProfileInfoBean.isEmpty() }">
		<thead> 
		<tr style="display: none;"><th></th></tr>
		</thead>
	</c:if>
	 <s:iterator value="viewedAndNotContactedProfileInfoBean" status="incr">
	 <tr><td>
	 <div class="profile_top"><a href="javascript:update(<s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['userId']}" />, <s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['profileId']}" />)">
      <span style="font-size: 20px;"><s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['fullName']}" /></span>
      <h2><s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['profileId']}" /> | Profile Created for <c:if test="${viewedAndNotContactedProfileInfoBean[incr.index]['profileFor'] == 'Myself'}">Self</c:if><c:if test="${viewedAndNotContactedProfileInfoBean[incr.index]['profileFor'] != 'Myself'}"><s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['profileFor']}" /></c:if></h2>
	    <c:choose>
			<c:when test="${viewedAndNotContactedProfileInfoBean[incr.index]['fileName'] != null}">
			<c:if test="${viewedAndNotContactedProfileInfoBean[incr.index]['showProfilePicture'] == 'false'}">
					<c:if test="${viewedAndNotContactedProfileInfoBean[incr.index]['gender'] == 'Male'}">
						<div class="col-sm-3 profile_left-top">
							<img width="200" height="150" src="<%=request.getContextPath()%>/images/Male_Pic_Producted.png" />
						</div>
					 </c:if>
					 <c:if test="${viewedAndNotContactedProfileInfoBean[incr.index]['gender'] == 'Female'}">
						<div class="col-sm-3 profile_left-top">
							<img width="200" height="150" src="<%=request.getContextPath()%>/images/Female_Pic_Producted.png" />
						</div>
					 </c:if>
					 </c:if>
					 <c:if test="${viewedAndNotContactedProfileInfoBean[incr.index]['showProfilePicture'] == 'true'}">
					<div class="col-sm-3 profile_left-top">
				    	<img width="200" height="150" src="<s:url action="ImageAction?imageId=%{viewedAndNotContactedProfileInfoBean[#incr.index]['fileName']}" />" class="img-responsive" alt=""/>
				    </div>
				    </c:if>
			</c:when>
			<c:otherwise>
	    ...
	    <div class="col-sm-3">
	      <ul class="login_details1">
			 <li>About me :</li>
			 <li><p><s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['aboutMe']}" /></p></li>
		  </ul>
	    </div>
	    <div class="col-sm-6">
	    	<table class="table_working_hours">
	        	<tbody>
	        		<tr class="opened_1">
						...
						<td class="day_value"><s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['age']}" />, <s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['height']}" /></td>
					</tr>
				    <tr class="opened">
						<td class="day_label1">Religion :</td>
						<td class="day_value"><s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['religion']}" /></td>
					</tr>
				    <tr class="opened">
						<td class="day_label1">Marital Status :</td>
						<td class="day_value"><s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['maritalStatus']}" /></td>
					</tr>
				    <tr class="opened">
						<td class="day_label1">Location :</td>
						<td class="day_value"><s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['country']}" /></td>
					</tr>
				    <tr class="closed">
						<td class="day_label1">Profile Created by :</td>
						<td class="day_value closed"><span><s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['profileFor']}" /></span></td>
					</tr>
				    <tr class="closed">
						<td class="day_label1">Education :</td>
						<td class="day_value closed"><span><s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['education']}" /></span></td>
					</tr>
			    </tbody>
		   </table>
		   <div class="buttons">
<!-- 			   <div class="vertical">Send Mail</div> -->
			   <c:if test="${viewedAndNotContactedProfileInfoBean[incr.index]['shortlisted'] != true}">
			   <div class="vertical" id="shortlist<s:property value="%{#incr.index}" />">
			   <a style="color: #fff;" href="javascript:shortlist('<s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['fullName']}" />', <s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['userId']}" />, <s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['profileId']}" />, 'Shortlist', <s:property value="%{#incr.index}" />)">
			   Shortlist
			   </a>
			   </div>
			</c:if>
			<c:if test="${viewedAndNotContactedProfileInfoBean[incr.index]['shortlisted'] == true}">
			   ...
			   <a style="color: #fff;" href="javascript:shortlisted('<s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['fullName']}" />', <s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['userId']}" />, <s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['profileId']}" />, 'Shortlisted', <s:property value="%{#incr.index}" />)">
			   Shortlisted
			   </a>
			   </div>
			</c:if>
			...
			<c:if test="${viewedAndNotContactedProfileInfoBean[incr.index]['sendInterested'] != true}">
			   <div class="vertical" id="sendInterest<s:property value="%{#incr.index}" />">
			   <a style="color: #fff;" href="javascript:sendinterest('<s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['fullName']}" />', <s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['userId']}" />, <s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['profileId']}" />, 'SendInterest', <s:property value="%{#incr.index}" />)">
			   Send Interest
			   </a></div>
			</c:if>
			<c:if test="${viewedAndNotContactedProfileInfoBean[incr.index]['sendInterested'] == true}">
			   <div class="vertical" id="sendInterested<s:property value="%{#incr.index}" />">
			   <a style="color: #fff;" href="javascript:sendreminder('<s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['fullName']}" />',<s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['userId']}" />, <s:property value="%{viewedAndNotContactedProfileInfoBean[#incr.index]['profileId']}" />, 'SendInterested', <s:property value="%{#incr.index}" />)">
			   Send Reminder
			   </a></div>
			</c:if>
