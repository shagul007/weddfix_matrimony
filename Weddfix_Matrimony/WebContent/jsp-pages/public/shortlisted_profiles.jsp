  ...
  <div class="container">
   <div class="breadcrumb1">
     <ul>
        ...
        <li class="current-page">Shortlisted Profiles</li>
     </ul>
   </div>
   <div class="col-md-9 profile_left2">
	 ...
	 <c:if test="${shortlistedProfileInfoBean.isEmpty() }">
	 	 <p class="control-lable1" for="sex">There are no profiles you were shortlisted. </p>
	 </c:if>
	 <table id="pagination">
		<c:if test="${!shortlistedProfileInfoBean.isEmpty() }">
			<thead> 
	 ...
	 <s:iterator value="shortlistedProfileInfoBean" status="incr">
	 <tr><td>
	 <div class="profile_top"><a href="javascript:update(<s:property value="%{shortlistedProfileInfoBean[#incr.index]['userId']}" />, <s:property value="%{shortlistedProfileInfoBean[#incr.index]['profileId']}" />)">
      <span style="font-size: 20px;"><s:property value="%{shortlistedProfileInfoBean[#incr.index]['fullName']}" /></span>
      <h2><s:property value="%{shortlistedProfileInfoBean[#incr.index]['profileId']}" /> | Profile Created for <c:if test="${shortlistedProfileInfoBean[incr.index]['profileFor'] == 'Myself'}">Self</c:if><c:if test="${shortlistedProfileInfoBean[incr.index]['profileFor'] != 'Myself'}"><s:property value="%{shortlistedProfileInfoBean[#incr.index]['profileFor']}" /></c:if></h2>
	    <c:choose>
			<c:when test="${shortlistedProfileInfoBean[incr.index]['fileName'] != null}">
			<c:if test="${shortlistedProfileInfoBean[incr.index]['showProfilePicture'] == 'false'}">
					<c:if test="${shortlistedProfileInfoBean[incr.index]['gender'] == 'Male'}">
						<div class="profile_left-top">
							<img width="200" height="150" src="<%=request.getContextPath()%>/images/Male_Pic_Producted.png" />
						</div>
					 </c:if>
					 <c:if test="${shortlistedProfileInfoBean[incr.index]['gender'] == 'Female'}">
						<div class="profile_left-top">
							<img width="200" height="150" src="<%=request.getContextPath()%>/images/Female_Pic_Producted.png" />
						</div>
					 </c:if>
					 </c:if>
					 <c:if test="${shortlistedProfileInfoBean[incr.index]['showProfilePicture'] == 'true'}">
					<div class="col-sm-3 profile_left-top">
				    	<img width="200" height="150" src="<s:url action="ImageAction?imageId=%{shortlistedProfileInfoBean[#incr.index]['fileName']}" />" class="img-responsive" alt=""/>
				    </div>
				    </c:if>
			</c:when>
			<c:otherwise>
	    ...
	    <div class="col-sm-3">
	      <ul class="login_details1">
			 <li>About me :</li>
			 <li><p><s:property value="%{shortlistedProfileInfoBean[#incr.index]['aboutMe']}" /></p></li>
		  </ul>
	    </div>
	    <div class="col-sm-6">
	    	<table class="table_working_hours">
	        	<tbody>
	        		<tr class="opened_1">
						...
						<td class="day_value"><s:property value="%{shortlistedProfileInfoBean[#incr.index]['age']}" />, <s:property value="%{shortlistedProfileInfoBean[#incr.index]['height']}" /></td>
					</tr>
				    <tr class="opened">
						<td class="day_label1">Religion :</td>
						<td class="day_value"><s:property value="%{shortlistedProfileInfoBean[#incr.index]['religion']}" /></td>
					</tr>
				    <tr class="opened">
						<td class="day_label1">Marital Status :</td>
						<td class="day_value"><s:property value="%{shortlistedProfileInfoBean[#incr.index]['maritalStatus']}" /></td>
					</tr>
				    <tr class="opened">
						<td class="day_label1">Location :</td>
						<td class="day_value"><s:property value="%{shortlistedProfileInfoBean[#incr.index]['country']}" /></td>
					</tr>
				    <tr class="closed">
						<td class="day_label1">Profile Created by :</td>
						<td class="day_value closed"><span><s:property value="%{shortlistedProfileInfoBean[#incr.index]['profileFor']}" /></span></td>
					</tr>
				    <tr class="closed">
						<td class="day_label1">Education :</td>
						<td class="day_value closed"><span><s:property value="%{shortlistedProfileInfoBean[#incr.index]['education']}" /></span></td>
					</tr>
			    </tbody>
		   </table>
		   <div class="buttons">
<!-- 			   <div class="vertical">Send Mail</div> -->
			 <c:if test="${shortlistedProfileInfoBean[incr.index]['shortlisted'] != true}">
			   <div class="vertical" id="shortlist<s:property value="%{#incr.index}" />">
			   <a style="color: #fff;" href="javascript:shortlist('<s:property value="%{shortlistedProfileInfoBean[#incr.index]['fullName']}" />', <s:property value="%{shortlistedProfileInfoBean[#incr.index]['userId']}" />, <s:property value="%{shortlistedProfileInfoBean[#incr.index]['profileId']}" />, 'Shortlist', <s:property value="%{#incr.index}" />)">
			   Shortlist
			   </a>
			   </div>
			</c:if>
			<c:if test="${shortlistedProfileInfoBean[incr.index]['shortlisted'] == true}">
			   ...
			   <a style="color: #fff;" href="javascript:shortlisted('<s:property value="%{shortlistedProfileInfoBean[#incr.index]['fullName']}" />', <s:property value="%{shortlistedProfileInfoBean[#incr.index]['userId']}" />, <s:property value="%{shortlistedProfileInfoBean[#incr.index]['profileId']}" />, 'Shortlisted', <s:property value="%{#incr.index}" />)">
			   Shortlisted
			   </a>
			   </div>
			</c:if>
			...
			<c:if test="${shortlistedProfileInfoBean[incr.index]['sendInterested'] != true}">
			   <div class="vertical" id="sendInterest<s:property value="%{#incr.index}" />">
			   <a style="color: #fff;" href="javascript:sendinterest('<s:property value="%{shortlistedProfileInfoBean[#incr.index]['fullName']}" />', <s:property value="%{shortlistedProfileInfoBean[#incr.index]['userId']}" />, <s:property value="%{shortlistedProfileInfoBean[#incr.index]['profileId']}" />, 'SendInterest', <s:property value="%{#incr.index}" />)">
			   Send Interest
			   </a></div>
			</c:if>
			<c:if test="${shortlistedProfileInfoBean[incr.index]['sendInterested'] == true}">
			   <div class="vertical" id="sendInterested<s:property value="%{#incr.index}" />">
			   <a style="color: #fff;" href="javascript:sendreminder('<s:property value="%{shortlistedProfileInfoBean[#incr.index]['fullName']}" />',<s:property value="%{shortlistedProfileInfoBean[#incr.index]['userId']}" />, <s:property value="%{shortlistedProfileInfoBean[#incr.index]['profileId']}" />, 'SendInterested', <s:property value="%{#incr.index}" />)">
			   Send Reminder
			   </a></div>
			</c:if>
