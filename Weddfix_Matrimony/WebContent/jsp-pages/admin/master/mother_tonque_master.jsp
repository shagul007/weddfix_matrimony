...
jQuery(document).ready(function() {
	jq(function() {		
		jq("#grid").jqGrid(
		{
		   	url : 'load_mother_tonque_master',
		   	editurl:"edit_mother_tonque_master",
		   	pageable: true,
		   	loadui:"enable",
			datatype: 'json',
			mtype: 'post',
		   	colNames:['Mother Tonque Id', 'Mother Tonque','Mother Tonque Description','Is Active'],
		   	colModel:[
		   		     {name:'id',align:'center', index:'id', width:100,editable:true,hidden:true,editoptions: {readonly:false,hidden:true,size:10}},
                    {name:'motherTonque',index:'motherTonque', width:100,editable:true,editrules:{required:true},editoptions:{size:10,maxlength: 30},searchoptions:{sopt: ['cn']}},
                    {name:'motherTonqueDesc',index:'motherTonqueDesc', width:100,editable:true,editrules:{required:true},editoptions:{size:10,maxlength: 250},searchoptions:{sopt: ['cn']}},
...
// 		    					jQuery('#motherTonqueName',form).attr('readonly','readonly');
		    				},afterSubmit: function (response, postdata) {
		                 		window.location.reload(true);
		                	 } },
		    		        {closeAfterAdd:true,beforeShowForm: function(form) {
// 		    					jQuery('#motherTonqueName',form).removeAttr('readonly');		                    
		                  	},
		                  	
		                  	
		                  	beforeSubmit:function(postdata,form){	
		                  		    var name=jQuery('#motherTonqueName').val();
		        					var mydata = jQuery("#grid").jqGrid('getGridParam','data');
		        					//alert('mydata='+mydata.length);
		        					for(var i=0;i<mydata.length;i++){
		        					if(re.test(name) == false){
		        		            		  return [false,'Mother Tonque should not have numbers or special characters'];
		        					}}
		        					return [true,','];
...
<body>
	<s:url var="remoteurl" action="load_mother_tonque_master" id="actionName" />
