...
jQuery(document).ready(function() {
	jq(function() {		
		jq("#grid").jqGrid(
		{
		   	url : 'load_weight_master',
		   	editurl:"edit_weight_master",
		   	pageable: true,
		   	loadui:"enable",
			datatype: 'json',
			mtype: 'post',
		   	colNames:['Weight Id', 'Weight','Weight Description','Is Active'],
		   	colModel:[
		   		     {name:'id',align:'center', index:'id', width:100,editable:true,hidden:true,editoptions: {readonly:false,hidden:true,size:10}},
                    {name:'weight',index:'weight', width:100,editable:true,editrules:{required:true},editoptions:{size:10,maxlength: 30},searchoptions:{sopt: ['cn']}},
                    {name:'weightDesc',index:'weightDesc', width:100,editable:true,editrules:{required:true},editoptions:{size:10,maxlength: 250},searchoptions:{sopt: ['cn']}},
...
// 		    					jQuery('#weight',form).attr('readonly','readonly');
		    				},afterSubmit: function (response, postdata) {
		                 		window.location.reload(true);
		                	 } },
		    		        {closeAfterAdd:true,beforeShowForm: function(form) {
// 		    					jQuery('#weight',form).removeAttr('readonly');		                    
		                  	},
		                  	...
		                  	beforeSubmit:function(postdata,form){	
		        					...
		        					if(re.test(name) == false){
		        		            		  return [false,'weight should not have numbers or special characters'];
		        					}}
		        					return [true,','];
...
<body>
	<s:url var="remoteurl" action="load_weight_master" id="actionName" />
