...
jQuery(document).ready(function() {
	jq(function() {		
		jq("#grid").jqGrid(
		{
		   	url : 'load_religion_master',
		   	editurl:"edit_religion_master",
		   	pageable: true,
		   	loadui:"enable",
			datatype: 'json',
			mtype: 'post',
		   	colNames:['Religion Id', 'Religion Name','Religion Description','Is Active'],
		   	colModel:[
		   		     {name:'id',align:'center', index:'id', width:100,editable:true,hidden:true,editoptions: {readonly:false,hidden:true,size:10}},
                    {name:'religionName',index:'religionName', width:100,editable:true,editrules:{required:true},editoptions:{size:10,maxlength: 30},searchoptions:{sopt: ['cn']}},
                    {name:'religionDesc',index:'religionDesc', width:100,editable:true,editrules:{required:true},editoptions:{size:10,maxlength: 250},searchoptions:{sopt: ['cn']}},
...
// 		    					jQuery('#religionName',form).attr('readonly','readonly');
		    				},afterSubmit: function (response, postdata) {
		                 		window.location.reload(true);
		                	 } },
		    		        {closeAfterAdd:true,beforeShowForm: function(form) {
// 		    					jQuery('#religionName',form).removeAttr('readonly');		                    
		                  	},
		                  	
		                  	
		                  	beforeSubmit:function(postdata,form){	
		                  		    var name=jQuery('#religionName').val();
		        					var mydata = jQuery("#grid").jqGrid('getGridParam','data');
		        					//alert('mydata='+mydata.length);
		        					for(var i=0;i<mydata.length;i++){
		        					if(re.test(name) == false){
		        		            		  return [false,'Religion Name should not have numbers or special characters'];
		        					}}
		        					return [true,','];
...
<body>
	<s:url var="remoteurl" action="load_religion_master" id="actionName" />
