...
jQuery(document).ready(function() {
	jq(function() {		
		jq("#grid").jqGrid(
		{
		   	url : 'load_occupation_master',
		   	editurl:"edit_occupation_master",
		   	pageable: true,
		   	loadui:"enable",
			datatype: 'json',
			mtype: 'post',
		   	colNames:['Occupation Id', 'Occupation','Occupation Description','Is Active'],
		   	colModel:[
		   		     {name:'id',align:'center', index:'id', width:100,editable:true,hidden:true,editoptions: {readonly:false,hidden:true,size:10}},
                    {name:'occupation',index:'occupation', width:100,editable:true,editrules:{required:true},editoptions:{size:10,maxlength: 30},searchoptions:{sopt: ['cn']}},
                    {name:'occupationDesc',index:'occupationDesc', width:100,editable:true,editrules:{required:true},editoptions:{size:10,maxlength: 250},searchoptions:{sopt: ['cn']}},
...
// 		    					jQuery('#occupation',form).attr('readonly','readonly');
		    				},afterSubmit: function (response, postdata) {
		                 		window.location.reload(true);
		                	 } },
		    		        {closeAfterAdd:true,beforeShowForm: function(form) {
// 		    					jQuery('#occupation',form).removeAttr('readonly');		                    
		                  	},
		                  	...
		                  	beforeSubmit:function(postdata,form){	
		        					...
		        					if(re.test(name) == false){
		        		            		  return [false,'Occupation should not have numbers or special characters'];
		        					}}
		        					return [true,','];
...
<body>
	<s:url var="remoteurl" action="load_occupation_master" id="actionName" />
